...
#define TAG "ti.tunegareceiver"

using namespace v8;

static Persistent<Object> bindingCache;

static void TiTuneGAReceiver_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "TiTuneGAReceiver.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::TiTuneGAReceiverBindings::lookupGeneratedInit(
...
static void TiTuneGAReceiver_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, TiTuneGAReceiver_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void TiTuneGAReceiver_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TiTuneGAReceiverBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TiTuneGAReceiverBinding = {
	"ti.tunegareceiver",
	TiTuneGAReceiver_init,
	TiTuneGAReceiver_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_ti_tunegareceiver_TiTuneGAReceiverBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.tunegareceiver", &TiTuneGAReceiverBinding);
	titanium::KrollBindings::addExternalLookup(&(::TiTuneGAReceiverBindings::lookupGeneratedInit));
}
